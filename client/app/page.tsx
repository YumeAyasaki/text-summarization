...
export default function Home() {
  ...
  const handleLoadFile = (
    ...
    if (file) {
      ...
      reader.onload = (e) =>
        onChange(e.target?.result?.toString() || '', index);
  ...
  return (
    <div className='flex h-full min-h-screen flex-col items-center bg-[#0E1117] px-4 pb-20 text-neutral-200'>
      ...
      <div className='mt-10 flex flex-col items-center justify-center'>
        <div className='mt-3 text-4xl font-bold'>Text summarization</div>
      </div>

      {inputs.map((input, index) => (
        <div key={index} className='mt-6 w-full'>
          <div className='flex-rol flex justify-between space-x-5 pb-3'>
            <div className='h-100 flex w-1/2 flex-col justify-center space-y-2'>
              <TextBlock
                editable={true}
                text={input}
                onChange={(newText) => handleInputChange(newText, index)}
              />
            </div>
            <div className='h-100 flex w-1/2 flex-col justify-center space-y-2'>
          ...
          <input
            type='file'
            onChange={(e) => handleLoadFile(e, handleInputChange, index)}
          />
          <button
            className='mx-2 w-[140px] cursor-pointer rounded-md bg-violet-500 px-4 py-2 font-bold hover:bg-violet-600 active:bg-violet-700'
